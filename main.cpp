...
private :
    ...
    static std::vector<ll> transferred_to;
    static std::vector <ld> transferred_value;
    static std::vector<ll> transferred_from;
...
public:
    ...
    void modify_client_name(std::string n, std::string pass) {
        name_ba = n;
        Password = pass;
    }
    ...
    void deposit() {
        ...
        while (true) {
            if (deposit_value == 0) {
                ...
                std::cout << "The transaction is completed successfully, your balance now is : " << balance
                          << std::endl;
                break;
            }
        }
    ...
    ld transfer(ld b, ll other, ll no) {
        ...
        while (true) {
            if (balance == 0) {
                std::cout << "You can't transfer money as your balance is Zero .. please try again" << std::endl
                          << std::endl;
                break;
            } else if (b == 0) {
                ...
                std::cout << "Sorry you can't transfer the money as there is only one account in the system"
                          << std::endl << std::endl;
                break;
            } else if (other == Client_acc_no_getter()) {
                std::cout << "You can't transfer money to yourself please enter number of bank account again"
                          << std::endl << std::endl;
                std::cin >> other;
                continue;
                ...
                std::cout << "The No. of this account doesn't exist please enter number of bank account again"
                          << std::endl << std::endl;
                std::cin >> other;
                continue;
            } else {
                ...
                transferred_to.push_back(other);
                transferred_from.push_back(account_number);
                transferred_value.push_back(b);
                std::cout << "The transaction is completed successfully, your balance now is : " << balance
                          << std::endl;
                break;
            }
    ...
    void view_transaction_report() {
        ll j, k;
        for (j = 0, k = 0; j < transact.size(); j++) {

            if (transact[j] == 'd') {
                std::cout << "#" << j + 1 << " The client has deposited: " << transact_value[j] << std::endl
                          << std::endl;
            } else if (transact[j] == 'w') {
                std::cout << "#" << j + 1 << " The client has withdrawn: " << transact_value[j] << std::endl
                          << std::endl;
            } else if (transact[j] == 't') {
                std::cout << "#" << j + 1 << " The client has transferred: " << transact_value[j] << " to client No. "
                          << transferred_to[k] << std::endl << std::endl;
                k++;
            }

        }
        for (ll w = 0; w < transferred_to.size(); w++) {
            if (account_number == transferred_to[w]) {
                std::cout << "#" << j + 1 << " The client No. " << transferred_from[w] << " transferred to him/her: "
                          << transferred_value[w] << std::endl << std::endl;
                j++;
            }
        }

        if (!transact.empty() || !transferred_to.empty())
            std::cout << "His/Her balance now is: " << balance << std::endl << std::endl;
    }
};

std::vector<ll> Bank_Account::transferred_to;
std::vector <ld> Bank_Account::transferred_value;
std::vector<ll> Bank_Account::transferred_from;
...
int main() {
    ...
    while (true) {
        std::cout
                << "Please enter\n(1) if you're a Developer\n(2) if you're a Client and want to open your back account\n(3) To END PROGRAM"
                << std::endl << std::endl;
        ...
        if (user_or_client == 1) {
            ...
            if (user_options == 2) {
                ...
                if (No_of_clients > clients_list.size() || No_of_clients < 0) {
                    ...
                    getline(std::cin, passwd);
                    std::cin >> age_of_client >> phone_of_client >> address_of_client >> is_client_married;
                    clients_list.at(No_of_clients - 1).Setter(name_of_client, age_of_client, phone_of_client,
                                                              address_of_client, is_client_married);
                    bankacc_list.at(No_of_clients - 1).modify_client_name(name_of_client, passwd);
                    //obj[product_number-1].report();
                    std::cout << "\n\nClient No. " << No_of_clients << " have been successfully modified" << std::endl
                              << std::endl;
                }
            }
            if (user_options == 3) {
                if (clients_list.empty())
                    ...
                    for (i = 0; i < clients_list.size(); i++) {
                        ...
                        bankacc_list[i].view_transaction_report();
...
//             cout<<"THIS "<<bankacc_list[nooo].Client_acc_no_getter();
                    ...
                    if (close == 'Y') {
                        ...
                        std::cout << "Error .. please make sure that you have entered whether (Y) or (N)" << std::endl
                                  << std::endl;
                        continue;
                    }
                }
            } else {
                std::cout << "Sorry .. The User Name or Password is incorrect .. please try again" << std::endl
                          << std::endl;
                continue;
            }
